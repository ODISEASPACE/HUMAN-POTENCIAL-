...
try {
    ...
    // Persistir el incremento del subnodo (SINTAXIS POSTGRESQL)
    $stmt = $pdo->prepare("
    ...
    // Actualizar la tabla de habilidades núcleo (SINTAXIS POSTGRESQL CORREGIDA)
    // EXCLUDED se usa en PostgreSQL para referirse al valor que se intentó insertar.
    $stmt = $pdo->prepare("
        INSERT INTO user_skills (user_id, node_key, current_level, unlocked) 
        VALUES (?, ?, ?, true) 
        ON CONFLICT (user_id, node_key) 
        DO UPDATE SET current_level = EXCLUDED.current_level, unlocked = true
    ");
    $stmt->execute([$user_id, $parent_key, $new_parent_level]);
